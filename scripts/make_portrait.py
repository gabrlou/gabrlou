...
FG = "#7ee787"         # terminal green, always — a terminal has no themes
BG = "#0d1117"         # the window's background
CARD = "#161b22"       # the title bar strip
TITLE = "gabrlou@github:~$ ./make_portrait.py"
...
def build_svg(lines, cols=COLS):
    ...
    p = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" '
         f'height="{height + TITLEBAR}" viewBox="0 0 {width} '
         f'{height + TITLEBAR}" font-family="{FAMILY}">',
         f'<style>.a{{fill:{FG}}}</style>',
         f'<rect width="{width}" height="{height + TITLEBAR}" rx="10" '
         f'fill="{BG}"/>',
         f'<rect width="{width}" height="{TITLEBAR}" rx="10" '
         f'fill="{CARD}"/>',
         f'<circle cx="16" cy="{TITLEBAR // 2}" r="4" fill="#f85149"/>',
         f'<circle cx="30" cy="{TITLEBAR // 2}" r="4" fill="#d29922"/>',
         f'<circle cx="44" cy="{TITLEBAR // 2}" r="4" fill="#3fb950"/>',
         f'<text x="58" y="{TITLEBAR // 2 + 4}" class="a" '
         f'font-size="9.5" opacity="0.6">{TITLE}</text>',
